...
public class CNNExtractorServiceImpl implements CNNExtractorService {
    ...
    @Override
    public String getPredictedLabel(Mat inputImage, Net dnnNet, String classesPath, Mat frame) {
        ...
        for (int i = 0; i < classification.rows(); ++i) {
            int left   = (int)(classification.get(i, 3)[0] * cols);
            int top    = (int)(classification.get(i, 4)[0] * rows);
            int right  = (int)(classification.get(i, 5)[0] * cols);
            int bottom = (int)(classification.get(i, 6)[0] * rows);
